            ...
            <form action="{{ route('students.update', $student->id) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 mb-4 text-center border-end">
                        <img src="https://ui-avatars.com/api/?name={{ $student->name }}&background=random&size=150" class="rounded-circle mb-3 shadow-sm" alt="profile">
                        <p class="text-muted">ID: #{{ $student->id }}</p>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label class="form-label text-muted small fw-bold">ชื่อนักเรียน (Name)</label>
                            <input type="text" name="name" class="form-control form-control-lg bg-light" value="{{ $student->name }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label text-muted small fw-bold">อีเมล (Email)</label>
                            <input type="email" name="email" class="form-control form-control-lg bg-light" value="{{ $student->email }}">
                        </div>
